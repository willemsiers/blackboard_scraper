...
BASE = "https://blackboard.utwente.nl"
...
class BlackboardUnit():
    ...
    def fetch_document(self, file_url, folder_name, path):
        print("fetching doc in ", folder_name, "("+file_url+")")
        ...
        if name != 'defaultTab' and '.html' not in name:
            ...
            if (not os.path.exists(thepath + name)):
                ...
                if urlResponse.status_code == requests.codes.ok:
                    ...
                    print("Skipping file because already exists on disk")
                    return False
    ...
    def recursiveScrape(self, content_id, folder_name, visitlist, path):
        print("====> recurse at ", folder_name)

        url = \
            BASE + '/webapps/blackboard/content/listContent.jsp?course_id=_' \
        ...
        for htmlLink in soup.find_all('a'):
            ...
            if '.pdf' in link  or '.doc' in link or 'ppt' in link or 'xid' in link:
                ...
                link = link.replace('blackboard.utwente.nl/', '')
                name = htmlLink.text
                ...
                try:
                    if(link[0] == '/'):
                        link = BASE +  link
                    self.fetch_document(link, folder_name, path)
                except:
                    print("Error: %s -  %s" % (sys.exc_info()[0], str(sys.exc_info()[1])))

        for htmlLink in soup.find_all('a'):
            link = htmlLink.get('href')
            if link.startswith(BASE + '/webapps/blackboard/content/listContent.jsp?') or link.startswith('/webapps/blackboard/content/listContent.jsp?'):
                link = link.replace(BASE + '/webapps/blackboard/content/listContent.jsp?course_id=_'
    ...
    def startScrape(self, path):
        print("==> startScrape")
        visitlist = []
        request = self.session.get(BASE + '/webapps/blackboard/execute/launcher?type=Course&id=_' + self.uid + '_1')
        soup = BeautifulSoup(request.text, "html.parser")
        for htmlLink in soup.find_all('a'):
            link = htmlLink.get('href')
            if '.pdf' in link or 'xid' in link:
                link.replace(BASE, '')
                link.replace('blackboard.utwente.nl', '')
                ...
                try:
                    self.fetch_document(BASE + link, '', path)
                except:
                    print("Error: %s -  %s" % (sys.exc_info()[0], str(sys.exc_info()[1])))
            elif link.startswith(BASE + '/webapps/blackboard/content/listContent.jsp?') or link.startswith('/webapps/blackboard/content/listContent.jsp?'):
                link = link.replace(BASE + '/webapps/blackboard/content/listContent.jsp?course_id=_'
                               + self.uid + '_1&content_id=_', '')
                link = link.replace('/webapps/blackboard/content/listContent.jsp?course_id=_'
                               + self.uid + '_1&content_id=_', '')
...
class BlackboardSession():
    ...
    def getUnitList(self):
        ...
        response = self.session.post(BASE + '/webapps/portal/execute/tabs/tabAction', data=payload);
        # response = self.session.get(BASE + '/webapps/portal/execute/tabs/tabAction?tab_tab_group_id=_1_1')

        soup = BeautifulSoup(response.text, "lxml")
        for htmlLink in soup.find_all('a'):
            link = htmlLink.get('href')

            if 'launcher?type=Course' in link:
                link = \
    ...
    def getILectureList(self):
        try:
            for unit in self.unitList:
                request = \
                    self.session.get(BASE + '/webapps/blackboard/execute/launcher?type=Course&id=_'
                           + unit.uid + '_1')
                soup = BeautifulSoup(request.text, "html.parser")
...
class scrapergui(Frame):
    ...
    def __on_Button5_ButRel_1(self, Event=None):
        for lecs in map(int, self.__Listbox2.curselection()):
            ...
            webbrowser.open(BASE + url, new=1,
                            autoraise=True)
